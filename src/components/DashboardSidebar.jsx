...
export default function DashboardSidebar({ activePage = "Dashboard", user }) {
  ...
  const linkStyle = (page) => ({
    ...
    color: '#1A1918',
    background: isActive(page) ? 'rgba(0,0,0,0.08)' : 'transparent',
    fontWeight: isActive(page) ? 600 : 450,
  });
  const ic = '#9B9A98';

  // Icon wrapper — no background, just consistent sizing
  const iconBox = (svg) => (
    <div style={{
      width: 24, height: 24,
  ...
  const navLinks = (
    <>
      ...
      <Link to={createPageUrl("CreateOffer")} onClick={() => setMobileMenuOpen(false)} style={linkStyle('CreateOffer')}>
        {iconBox(<svg width="16" height="16" viewBox="0 0 24 24" fill={ic}><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm5 11h-4v4h-2v-4H7v-2h4V7h2v4h4v2z"/></svg>)}
        Create Loan
      </Link>
      ...
      <Link to={createPageUrl("Upcoming")} onClick={() => setMobileMenuOpen(false)} style={linkStyle('Upcoming')}>
        {iconBox(<svg width="16" height="16" viewBox="0 0 24 24" fill={ic}><path d="M17 12h-5v5h5v-5zM16 1v2H8V1H6v2H5C3.89 3 3.01 3.9 3.01 5L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-1V1h-2zm3 18H5V8h14v11z"/></svg>)}
        Upcoming
      </Link>
      ...
      <Link to={createPageUrl("YourLoans")} onClick={() => setMobileMenuOpen(false)} style={linkStyle('YourLoans')}>
        {iconBox(<svg width="16" height="16" viewBox="0 0 24 24" fill={ic}><path d="M16 6l2.29 2.29-4.88 4.88-4-4L2 16.59 3.41 18l6-6 4 4 6.3-6.29L22 12V6z"/></svg>)}
        Lending & Borrowing
      </Link>
      ...
      <Link to={createPageUrl("RecentActivity")} onClick={() => setMobileMenuOpen(false)} style={linkStyle('RecentActivity')}>
        {iconBox(<svg width="16" height="16" viewBox="0 0 24 24" fill={ic}><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z"/></svg>)}
        Recent Activity
      </Link>
      ...
      <Link to={createPageUrl("LoanAgreements")} onClick={() => setMobileMenuOpen(false)} style={linkStyle('LoanAgreements')}>
        {iconBox(<svg width="16" height="16" viewBox="0 0 24 24" fill={ic}><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg>)}
        Loan Documents
      </Link>
    </>
  );
  ...
  const sidebarFooter = (
    <div style={{ padding: '8px 12px 12px', borderTop: '1px solid rgba(0,0,0,0.07)', display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <Link to={createPageUrl("Requests")} onClick={() => setMobileMenuOpen(false)} style={{ ...linkStyle('Requests'), position: 'relative' }}>
        {iconBox(<svg width="16" height="16" viewBox="0 0 24 24" fill={ic}><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.63-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.64 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg>)}
        Notifications
        {notifCount > 0 && (
  ...
  return (
    <>
      ...
      <div className="mobile-header" style={{
        ...
        {/* Right: avatar */}
        <Link to={createPageUrl("Profile")} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, borderRadius: '50%', background: '#82F0B9', color: 'white', fontWeight: 700, fontSize: 12, textDecoration: 'none', flexShrink: 0 }}>
          {avatarInitial}
        </Link>
      </div>
      ...
      <div className="home-sidebar" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <Link to={createPageUrl("Requests")} style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', width: 34, height: 34, borderRadius: 8, textDecoration: 'none' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="white"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.63-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.64 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg>
            {notifCount > 0 && (
              <div style={{ position: 'absolute', top: 2, right: 2, background: '#E8726E', color: 'white', fontSize: 9, fontWeight: 700, minWidth: 15, height: 15, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px', lineHeight: 1 }}>
                {notifCount > 99 ? '99+' : notifCount}
              </div>
            )}
          </Link>
          <Link to={createPageUrl("Profile")} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 34, height: 34, borderRadius: 8, textDecoration: 'none' }}>
      ...
      <aside className="home-sidebar" style={{
        position: 'fixed', left: 0, top: 44, bottom: 0, width: 240,
        background: '#EDF4FB',
        zIndex: 52, display: 'flex', flexDirection: 'column',
        fontFamily: "'DM Sans', sans-serif", overflowY: 'auto',
        padding: '12px 12px 16px',
      }}>
        {/* Nav + Settings wrapped in glass-carousel-frame box */}
        <div className="glass-carousel-frame" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'visible' }}>
          <nav style={{ flex: 1, padding: '4px 8px 4px', display: 'flex', flexDirection: 'column', gap: 2 }}>
            {navLinks}
          </nav>
          {/* Settings footer inside box */}
          <div style={{ padding: '4px 8px 6px', borderTop: '1px solid rgba(0,0,0,0.05)', flexShrink: 0 }}>
            <div ref={settingsRef} style={{ position: 'relative' }}>
