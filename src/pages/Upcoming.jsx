          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {overdue.length > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '7px 0' }}>
                ...
                <button onClick={() => setResolveModal({ loan: overdue[0].loan, loans: overdue.map(e => e.loan) })}
              ...
              return dayEvents.map((event, ei) => (
                <div key={`${i}-${ei}`} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '7px 0' }}>
                  <div style={{ width: 52, flexShrink: 0, opacity: ei === 0 ? 1 : 0 }}>
                    <div style={{ fontSize: 10, fontWeight: 500, color: isToday ? '#03ACEA' : '#9B9A98' }}>{isToday ? 'Today' : format(day, 'EEE')}</div>
                    <div style={{ fontSize: 12, fontWeight: 500, color: '#1A1918' }}>{format(day, 'MMM d')}</div>
                  </div>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 500, color: '#1A1918', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {event.isLender ? `Expect ${formatMoney(event.amount)} from ${event.firstName}` : `${formatMoney(event.amount)} due to ${event.firstName}`}
                    </div>
                    {event.reason && <div style={{ fontSize: 10, color: '#9B9A98', marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{event.reason}</div>}
                  </div>
                </div>
              ));
            })}
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          ...
          {comingLater.length > 0 && (
            <span style={{ fontSize: 11, color: '#9B9A98' }}>{comingLater.length} · {formatMoney(comingLater.reduce((s, e) => s + e.amount, 0))}</span>
          )}
        </div>
        {comingLater.length === 0 ? (
