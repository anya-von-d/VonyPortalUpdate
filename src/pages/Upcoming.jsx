        ...
        <div className="mesh-center" style={{ background: 'transparent', padding: '24px 56px 80px' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            <div style={{ display: 'flex', background: '#F0F0EE', borderRadius: 12, padding: 3, gap: 2 }}>
              {[
                { key: 'summary', label: 'Schedule' },
                { key: 'plan', label: 'Plan Your Month' },
              ].map(({ key, label }) => {
          ...
          {activeTab === 'plan' && (() => {
            ...
            safePayments.forEach(p => {
              ...
              cashLines.push({ id: `paid-${p.id}`, label: isLender ? `Received from ${name} · ${paidDateStr}` : `Sent to ${name} · ${paidDateStr}`, amount: isLender ? (p.amount || 0) : -(p.amount || 0), date: d, status: 'done', reason: paidReason });
            });
