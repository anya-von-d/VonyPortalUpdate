...
const TABS = [
  { key: 'all', label: 'All' },
  { key: 'attention', label: 'Offers that need your attention' },
...
function CircleIcon({ bg, children }) {
  return (
    <div style={{
...
// ── Detail sheet for pending items (loan offer sent / payment sent) ───────────
...
export default function Notifications() {
  ...
  const getUserById = (userId) => profiles.find(p => p.user_id === userId) || null;
  ...
  const buildAllItems = () => {
    ...
    reminders.forEach(reminder => {
      ...
      items.push({
        ...
        subtitle: `${formatMoney(reminder.paymentAmount)}${Math.abs(reminder.daysUntil) <= 0 ? '' : ` · ${Math.abs(reminder.daysUntil)} day${Math.abs(reminder.daysUntil) !== 1 ? 's' : ''} ${isOverdue ? 'overdue' : 'away'}`}`,
        bg: isOverdue ? '#FEE2E2' : '#DBEAFE',
        iconColor: isOverdue ? '#E8726E' : '#03ACEA',
        isOverdue,
      });
    });
  ...
  const getTabItems = (tab) => {
    ...
    if (tab === 'support') return allItems.filter(i => i.category === 'support');
    if (tab === 'priority') return allItems.filter(i => i.isOverdue || i.type === 'payment_confirm');
    return allItems;
  };
  ...
  const handleItemTap = (item) => {
    if (item.type === 'friend') {
      window.dispatchEvent(new CustomEvent('open-friends-popup', { detail: { initialRequestsOpen: true } }));
    } else if (item.type === 'offer_received') {
      setSelectedOffer(item.data);
      setShowSignatureModal(true);
    } else if (item.type === 'payment_confirm') {
      navigate(createPageUrl('RecordPayment'));
    } else if (item.type === 'term_change') {
      navigate(createPageUrl('LendingBorrowing'));
    } else if (item.type === 'loan_sent') {
      setSelectedPendingRow({ type: 'loan_sent', loan: item.loan, profile: item.profile });
    } else if (item.type === 'payment_sent') {
      setSelectedPendingRow({ type: 'payment_sent', payment: item.payment, loan: item.loan, profile: item.profile });
    } else if (item.isOverdue) {
      navigate(createPageUrl('RecordPayment'));
    } else {
      navigate(createPageUrl('Upcoming'));
    }
  };

  // ── Icon renderer ─────────────────────────────────────────────────────────
  const renderIcon = (item) => {
    const { type, iconColor } = item;
    if (type === 'friend') return (
      <CircleIcon bg={item.bg}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
      </CircleIcon>
    );
    if (type === 'offer_received') return (
      <CircleIcon bg={item.bg}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/></svg>
      </CircleIcon>
    );
    if (type === 'payment_confirm') return (
      <CircleIcon bg={item.bg}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
      </CircleIcon>
    );
    if (type === 'term_change') return (
      <CircleIcon bg={item.bg}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
      </CircleIcon>
    );
    if (type === 'loan_sent' || type === 'payment_sent') return (
      <CircleIcon bg={item.bg}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
      </CircleIcon>
    );
    // reminders
    const isOverdue = type.startsWith('overdue');
    if (type.includes('owe')) return (
      <CircleIcon bg={item.bg}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
      </CircleIcon>
    );
      ...
      <div className="mesh-layout" style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 0, minHeight: '100vh' }}>
        ...
        <div style={{ padding: '80px 20px 120px' }}>
          ...
          <div style={{
            ...
            <style>{`.notif-tab-scroll::-webkit-scrollbar { display: none; }`}</style>
            {TABS.map(tab => {
